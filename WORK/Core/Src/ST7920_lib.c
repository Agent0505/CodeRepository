...
extern SPI_HandleTypeDef hspi2; //Используемая шина spi
extern DMA_HandleTypeDef hdma_spi2_tx; //Задействован модуль DMA. Подключена отправка по 8 bit.
...
#define cs_set() HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, 1);          //CS притягиваем к 3.3v
#define cs_reset() HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, 0);   	  //CS_притягиваем к земле
#define RST_set() HAL_GPIO_WritePin(RST_GPIO_Port, RST_Pin, 1);       //RST притягиваем к 3.3v
#define RST_reset() HAL_GPIO_WritePin(RST_GPIO_Port, RST_Pin, 0);     //RST притягиваем к земле
